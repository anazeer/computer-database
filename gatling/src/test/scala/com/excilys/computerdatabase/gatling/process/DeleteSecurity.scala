...
object DeleteSecurity {
  ...
  val delete = exec(http("Search for delete")
    .get(new StringBuilder().append(config.getString("application.urls.dashboardPage")).append("?").append(config.getString("application.urls.param.search")).append("=${addComputerName}-edited").toString())
    .check(
      css("#${addComputerName}-edited_id", "value").saveAs("computerId"),
